...
Natrix.Core is the experimental component layer for Natrix. It builds on Natrix.StdWeb and Natrix.Signals to provide components, typed DOM component wrappers, reactive rendering, browser rendering roots, and server-side rendering primitives.
...
- Host setup for browser WebAssembly client apps through `NatrixHostBuilder`.
...
using Natrix.Core.DomComponents;
...
A browser WebAssembly client app should initialize StdWeb, resolve the root DOM element, and mount the component tree through `NatrixHostBuilder`. The host sets up the feature context that components expect while mounting.
...
See [../Natrix.CoreExample](../Natrix.CoreExample/) for a browser WebAssembly client app using signals, DOM components, refs, events, and `ForEach`.
